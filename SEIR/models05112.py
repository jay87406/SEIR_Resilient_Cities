...
class SEIRSModel():
    ...
    def __init__(self, initN, beta, sigma, gamma, xi=0, mu_I=0, mu_0=0, nu=0, p=0,
        ...
        self.numS = numpy.array([self.N[-1] - self.numE[-1] - self.numI[-1] - self.numQ_E[-1] - self.numQ_I[-1] -
    ...
    def run_epoch(self, runtime, dt=0.1):
        ...
        init_cond = [self.numS[-1], self.numE[-1], self.numI[-1], self.numQ_E[-1], self.numQ_I[-1], self.numR[-1],
                     self.numF[-1]]
        ...
        solution = scipy.integrate.solve_ivp(
            lambda t, X: SEIRSModel.system_dfes(t, X, self.beta, self.sigma, self.gamma, self.xi, self.mu_I, self.mu_0,
                                                self.nu,
                                                self.beta_Q, self.sigma_Q, self.gamma_Q, self.mu_Q, self.theta_E,
    ...
    def run(self, T, dt=0.1, checkpoints=True, verbose=True):
        ...
        if (not checkpoints):
            ...
            print("t = %.2f" % self.t)
            print("MAX_I   = " + str((max(self.numI)) / self.N))
            if (verbose):
    ...
    def plot(self, ax=None, plot_S='line', plot_E='line', plot_I='line', plot_R='line', plot_F='line',
             ...
             ylim=None, xlim=None, legend=True, title=None, side_title=None, plot_percentages=None):
        ...
        #numpy.savetxt(r'0730.txt', Iseries, fmt='%d')
        ...
        if (any(Iseries) and plot_I == 'stacked'):
            ...
            # 0801------- 修改储存txt
            # numpy.ma.masked_where(Iseries <= 0, self.tseries) 时间
            # numpy.ma.masked_where(Iseries <= 0, Iseries) 数值
            print(Iseries[0])
            test = []
            test = [0] * len(Iseries)
            j=0
            ...
            # 0808修改新增资料夹
            # time_m_d = time.strftime("%m-%d", time.localtime())
            # os.makedirs('./' + time_m_d, exist_ok=True)
    ...
    def figure_infections(self, plot_S=False, plot_E=False, plot_I='stacked', plot_R=False, plot_F=False,
                          plot_Q_E=False, plot_Q_I=False, combine_Q=False,
                          color_S='tab:green', color_E='orange', color_I='crimson', color_R='tab:blue', color_F='black',
                          color_Q_E='mediumorchid', color_Q_I='mediumorchid', color_reference='#E0E0E0',
                          dashed_reference_results=None, dashed_reference_label='reference',
                          shaded_reference_results=None, shaded_reference_label='reference',
                          vlines=[], vline_colors=[], vline_styles=[], vline_labels=[],
                          ylim=None, xlim=None, legend=False, title=None, side_title=None, plot_percentages=None,
                          figsize=(12, 8), use_seaborn=True, show=True):
...
class SEIRSNetworkModel0511():
    ...
    def plot(self, ax=None, plot_S='line', plot_E='line', plot_I='line', plot_R='line', plot_F='line',
             ...
             shaded_reference_results=None, shaded_reference_label='reference',
             vlines=[], vline_colors=[], vline_styles=[], vline_labels=[],
             ylim=None, xlim=None, legend=True, title=None, side_title=None, plot_percentages=True, start_time='2022/03/1', simulation_area='Shanghai'):
        ...
        # 0715 呼叫SEIR class函数 用Fseries取代
        #-----------
        # Fseries = SEIRSModel(Iseries)
        # 0731 修改  ------VVV
        Fseries = numpy.loadtxt('0730.txt', dtype=int)
        #if (any(Fseries) and plot_F == 'line'):
        #    ax.plot(numpy.ma.masked_where(Fseries <= 0, self.tseries), numpy.ma.masked_where(Fseries <= 0, Fseries*100),
        #           color=color_F, label='$F$', zorder=6)
        if (any(Fseries) and plot_F == 'line'):
            ax.plot(Fseries*1000, color=color_F, label='$Basic SEIR$', zorder=6)
        # 0731      -----^^^^
        # 0801 -------------VVVV
        # 修改增加 EX数据
        Sseries = numpy.loadtxt('0730_ex_seir.txt', dtype=int)
        # 0801 -------------^^^^


        #-----------
        # 0630 把 真实数据画图 目前是把E 的线条取代 后续要补回去
        # 上海全 3/25开始数据
        #Eseries = numpy.array([2270, 2676, 3500, 4477, 5982, 5653,4502, 6311,8226,9006,13354,17077,19922,21222,23624,24943,26087,23342,26330,27719,23072,23513,24820,22248,20416,18901,18495,17629,23370,21058,19455,16980,13562,10622,15032,10181,7872,7333,5669,4982,4651,4269,4214,3975,3947,3014,1487,1449,2096,1681,1369,938,823,855,719,860,868,622,558,480,387,338,264,170,99,70,27,15,16,13,15,16,8,10,15,9,11,16,29,37,17,15,16,4,7,9,13,9,8])
        # Eseries = numpy.array(
        #     [121, 176, 241, 321, 396, 479, 544, 713, 852, 1054, 1212, 1472, 2270, 2676, 3500, 4477, 5982, 5653, 4502, 6311, 8226, 9006, 13354, 17077, 19922, 21222, 23624, 24943,
        #      26087, 23342, 26330, 27719, 23072, 23513, 24820, 22248, 20416, 18901, 18495, 17629, 23370, 21058, 19455,
        #      16980, 13562, 10622, 15032, 10181, 7872, 7333, 5669, 4982, 4651, 4269, 4214, 3975, 3947, 3014, 1487, 1449,
        #      2096, 1681, 1369, 938, 823, 855, 719, 860, 868, 622, 558, 480, 387, 338, 264, 170, 99, 70, 27, 15, 16, 13,
        #      15, 16, 8, 10, 15, 9, 11, 16, 29, 37, 17, 15, 16, 4, 7, 9, 13, 9, 8])
        if(simulation_area=='Shanghai'):
            Eseries = numpy.loadtxt('./16/' + 'Shanghai.txt', delimiter=',', dtype=int)
        if (simulation_area == 'huangpuqu'):
            Eseries = numpy.loadtxt('./16/' + 'huangpuqu.txt', delimiter=',', dtype=int)
        if (simulation_area == 'pudong'):
            Eseries = numpy.loadtxt('./16/' + 'pudong.txt', delimiter=',', dtype=int)
        if (simulation_area == 'putuoqu'):
            Eseries = numpy.loadtxt('./16/' + 'putuoqu.txt', delimiter=',', dtype=int)
        if (simulation_area == 'songjiang'):
            Eseries = numpy.loadtxt('./16/' + 'songjiang.txt', delimiter=',', dtype=int)
        if (simulation_area == 'xuhuiqu'):
            Eseries = numpy.loadtxt('./16/' + 'xuhuiqu.txt', delimiter=',', dtype=int)
        if (simulation_area == 'changningqu'):
            Eseries = numpy.loadtxt('./16/' + 'changningqu.txt', delimiter=',', dtype=int)
        if (simulation_area == 'jinganqu'):
            Eseries = numpy.loadtxt('./16/' + 'jinganqu.txt', delimiter=',', dtype=int)
        if (simulation_area == 'hongkouqu'):
            Eseries = numpy.loadtxt('./16/' + 'hongkouqu.txt', delimiter=',', dtype=int)
        if (simulation_area == 'yangpuqu'):
            Eseries = numpy.loadtxt('./16/' + 'yangpuqu.txt', delimiter=',', dtype=int)
        if (simulation_area == 'xingqu'):
            Eseries = numpy.loadtxt('./16/' + 'xingqu.txt', delimiter=',', dtype=int)
        if (simulation_area == 'baoshanqu'):
            Eseries = numpy.loadtxt('./16/' + 'baoshanqu.txt', delimiter=',', dtype=int)
        if (simulation_area == 'jiading'):
            Eseries = numpy.loadtxt('./16/' + 'jiading.txt', delimiter=',', dtype=int)
        if (simulation_area == 'jinshan'):
            Eseries = numpy.loadtxt('./16/' + 'jinshan.txt', delimiter=',', dtype=int)
        if (simulation_area == 'chongming'):
            Eseries = numpy.loadtxt('./16/' + 'chongming.txt', delimiter=',', dtype=int)
        if (simulation_area == 'qingpu'):
            Eseries = numpy.loadtxt('./16/' + 'qingpu.txt', delimiter=',', dtype=int)
        if (simulation_area == 'fengxian'):
            Eseries = numpy.loadtxt('./16/' + 'fengxian.txt', delimiter=',', dtype=int)
        #
        # 徐汇区Eseries = numpy.array([3,331,277,91,1090,404,226,639,1042,499,1229,920,1107,2076,1629,1150,3203,1171,1108,1492,1342,1689,1532,1553,572,1608,1447,1366,1275,1722,1396,1341,904,552,1580,602,488,442,417,363,330,323,330,299,306,128,83,74,93,105,89,63,52,51,44,37,41,42,42,27,22,21,18,12,10,5])
        # 松江区Eseries = numpy.array([15,79,190,94,228,246,184,493,567,263,559,796,781,751,771,500,1825,691,712,657,761,773,410,905,762,325,676,493,2568,773,548,641,287,231,402,134,105,96,68,90,92,83,27,48,40,26,19,14,22,11,6,14,11,19,15,10,6,13,5,7,1,0,2,4,0])
        # 普陀区Eseries = numpy.array([9,69,70,81,113,146,15,245,388,321,254,483,1033,957,1094,653,1001,1878,1170,475,24,426,1331,1225,371,528,555,489,523,478,477,418,365,301,235,141,114,95,79,62,91,79,77,56,49,50,30,29,29,28,28,8,10,12,13,20,20,14,12,12,9,9,6,4,0,2])
        # 上海全 3/1开始数据 Eseries = numpy.array([2, 10, 26, 45, 73, 121, 176, 241, 321, 396, 479, 544, 713, 852, 1054, 1212, 1472, 1846, 2355, 3113, 4009, 4990, 5973, 7582, 9852, 12528, 16028, 20505, 26487, 32140, 36642])
        #浦东的 Eseries = numpy.array([135, 220, 169, 237, 218, 193, 1914, 323, 1429, 2506,2183, 2207, 2407, 2584, 2038, 3654, 7071, 8145, 8457, 9050, 7286, 11130, 6732, 8306, 11049, 15027, 11656,10282, 10791, 7740, 8831, 5646, 4465, 4655, 7961, 7626, 6181, 3912, 2745, 2993, 3472, 2028, 1625, 1652,1162, 883, 889, 677, 584, 675, 692, 588, 263, 250, 280, 187, 150, 98, 89, 83, 75, 82, 78, 75, 59, 60, 47,44, 35, 28, 18, 12, 6, 4])
        if (any(Eseries) and plot_E == 'line'):
            ax.plot(Eseries,
                    color=color_E, label='$COVID-19 Data$', zorder=6)
            print(Eseries)
        if (combine_D and (any(Dseries) and plot_Q_E == 'line' and plot_Q_I == 'line')):
            ...
            if (any(Q_Iseries) and plot_Q_I == 'line'):
                ax.plot(numpy.ma.masked_where(Q_Iseries <= 0, self.tseries),
                        numpy.ma.masked_where(Q_Iseries <= 0, Q_Iseries), color=color_Q_I, label='$Q_I$', zorder=6)
        if (any(Iseries) and plot_I == 'line'):
            #0802ax.plot(numpy.ma.masked_where(Iseries <= 0, self.tseries), numpy.ma.masked_where(Iseries <= 0, Iseries*100),
            #0802        color=color_I, label='$Susceptible$', zorder=6)
            # 0807修改存社群网络txt
            test_net = []
            test_net = [0] * len(Iseries)
            k = 0
            for i in range(0, len(Iseries), 10):
                test_net[k] = Iseries[i]
                k = k + 1
                # print(Iseries[i])
            print(test_net)
            ...
            # timefile = time.strftime("%H-%M", time.localtime())
            # os.makedirs('./' + time_m_d + '/' + timefile, exist_ok=True)
            # numpy.savetxt(r'./' + time_m_d + '/' + timefile + '/0730_networks.txt', test_net, fmt='%d')
            # 这个用于显示plot的
            numpy.savetxt(r'0730_networks.txt', test_net, fmt='%d')
            # 把I平滑
            # -------
            def moving_average_I(interval, window_size):
        ...
        # 修改0703 因要用percentages 所以这里不用ax.set_xticklabels([DT + timedelta(days=x) for x in ax.get_xticks()])
        # 0630 原本是用于等比放大 ax.set_yticklabels(['{:.0f}'.format(y*100) for y in ax.get_yticks()])
    ...
    def figure_basic(self, plot_S='line', plot_E='line', plot_I='line', plot_R='line', plot_F='line',
                     plot_Q_E='line', plot_Q_I='line', combine_D=True,
                     color_S='tab:blue', color_E='orange', color_I='crimson', color_R='tab:blue', color_F='black',
                     color_Q_E='mediumorchid', color_Q_I='mediumorchid', color_reference='#E0E0E0',
                     dashed_reference_results=None, dashed_reference_label='reference',
                     shaded_reference_results=None, shaded_reference_label='reference',
                     vlines=[], vline_colors=[], vline_styles=[], vline_labels=[],
                     ylim=None, xlim=None, legend=None, title=None, side_title=None, plot_percentages=None,
                     figsize=(12, 8), use_seaborn=None, show=True, start_time='2022/03/1', simulation_area='Shanghai'):
        ...
        self.plot(ax=ax, plot_S=plot_S, plot_E=plot_E, plot_I=plot_I, plot_R=plot_R, plot_F=plot_F,
                  ...
                  vlines=vlines, vline_colors=vline_colors, vline_styles=vline_styles, vline_labels=vline_labels,
                  ylim=ylim, xlim=xlim, legend=legend, title=title, side_title=side_title,
                  plot_percentages=plot_percentages, start_time=start_time, simulation_area=simulation_area)
        if (show):
            pyplot.show()
