...
# 0218 修改
# num_cohorts 生成的组群层数
# num_nodes_per_cohort 每个队列的节点数 每个FARZ层的节点数，FARZ参数n
# num_teams_per_cohort 每个组群的团队数 每个FARZ层的社区数，FARZ参数k
# mean_intracohort_degree 每个人的队列内接触的平均数 每个FARZ层的平均程度，FARZ参数m
# pct_contacts_intercohort 每个员工在工作场所的总接触（总程度）中，属于同事间互动的百分比
# farz_params 指定FARZ网络生成器参数的字典，除了上面的参数n、k和m之外，其他参数都是由上面的参数给出的。
#       默认：{'alpha':5.0, 'gamma':5.0, 'beta':0.5, 'r':1, 'q':0.0, 'phi':1, 'b':0, 'epsilon':1e-6} 。
# distancing_scales 应该返回网络版本的社会疏远度列表
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

def generate_workplace_contact_network_0218(num_cohorts=3, num_nodes_per_cohort=100, num_teams_per_cohort=10,
    ...
    #下面 for 那段 ininstance 指的是jupyter里的参数更改
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    cohortFinalIdx = -1
    #0320修改增加了下面兩行參數
    pct_contacts_intercohort_0320_test = [0.2,0.5,0.8]
    test = 0
    for c, cohortNetwork in enumerate(cohortNetworks):
        ...
        for i in list(range(cohortNetwork.number_of_nodes())):
            ...
            i_interCohortDegree = int(
                ((1 / (1 - pct_contacts_intercohort_0320_test[test])) * i_intraCohortDegree) - i_intraCohortDegree)
        ...
        print(pct_contacts_intercohort_0320_test[test])
        test = test + 1
    ...
    if (layer_info is None):
        ...
        layer_info = {'0-9': {'ageBrackets': ['0-9'], 'meanDegree': 8.6, 'meanDegree_CI': (0.0, 17.7)},
                      ...
                      '20-59': {'ageBrackets': ['20-29', '30-39', '40-49', '50-59'],
                                'meanDegree': (
                                            (age_distn_given20to60['20-29'] + age_distn_given20to60['30-39']) * 15.3 + (
                                                age_distn_given20to60['40-49'] + age_distn_given20to60[
                                            '50-59']) * 13.8),
                                'meanDegree_CI': (((age_distn_given20to60['20-29'] + age_distn_given20to60[
                                    '30-39']) * 12.6 + (age_distn_given20to60['40-49'] + age_distn_given20to60[
                                    '50-59']) * 11.0), ((age_distn_given20to60['20-29'] + age_distn_given20to60[
                                    '30-39']) * 17.9 + (age_distn_given20to60['40-49'] + age_distn_given20to60[
                                    '50-59']) * 16.6))},
